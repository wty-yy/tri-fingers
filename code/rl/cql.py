...
class TrainConfig:
    # Experiment
    device: str = "cuda"
    env: str = "trifinger"  #"halfcheetah-medium-expert-v2"  # OpenAI gym environment name
    seed: int = 0  # Sets Gym, PyTorch and Numpy seeds
    eval_freq: int = 2e4 #int(5e3)  # How often (time steps) we evaluate
    n_episodes: int = 6 #10  # How many episodes run during evaluation
    max_timesteps: int = int(1e6)  # Max time steps to run environment
    checkpoints_path: Optional[str] = "/data/user/wanqiang/document/leibnizgym/models"  # Save path
    ...
    soft_target_update_rate: float = 5e-3 #5e-3  # Target network update rate    ############################
    target_update_period: int = 1 # 1  # Frequency of target nets updates       #########################
    ...
    cql_alpha: float = 10 #10.0  # Minimal Q weight
...
class ReplayBuffer:
    ...
    def load_d4rl_dataset(self, data: Dict[str, np.ndarray]):
        ...
        self._dones[:n_transitions]=torch.zeros([n_transitions,1],dtype=torch.float32,device=self._device)   #halfsheet 数据中都为0
...
class ContinuousCQL:
    ...
    def _policy_loss(
        ...
        if self.total_it <= self.bc_steps:
            ...
            policy_loss = (alpha * log_pi - q_new_actions).mean()  #让next的log_pi变小，（s，a）Q值变大
        return policy_loss
...
def train(hydra_cfg):
    ...
    max_action= float(1)
    print("target_entropy", -np.prod(env.action_space.shape).item())
    ...
    kwargs = {
        ...
        # CQL
        "target_entropy": -np.prod(env.action_space.shape).item(),  #-9
        "alpha_multiplier": config.alpha_multiplier,   #1.0
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            normalized_eval_score = eval_score * 100.0
