...
class ReplayBuffer:
    ...
    def load_d4rl_dataset(self, data: Dict[str, np.ndarray]):
        ...
        self._rewards[:n_transitions] = self._to_tensor(data["rewards"])*10
        self._next_states[:n_transitions] = self._to_tensor(data["next_observations"])
        #self._dones[:n_transitions] = self._to_tensor(data["terminals"])
        self._dones[:n_transitions]=torch.zeros([n_transitions,1],dtype=torch.float32,device=self._device)
