...
class TrifingerEnv(IsaacEnvBase):
 ...
 #########   _robot_urdf_file = "robot_properties_fingers/urdf/pro/trifingerpro.urdf"
    ...
    _max_velocity_radps = 3.14159268/4   #10
    # limits of the robot (mapped later: str -> torch.tensor)
    _robot_limits: dict = {
        "joint_position": SimpleNamespace(
            # matches those on the real robot
            low=np.array([-0.50, 0.3, -2.7] * _dims.NumFingers.value, dtype=np.float32),
            high=np.array([0.50, 1.57, -0.0] * _dims.NumFingers.value, dtype=np.float32),
    ...
    _robot_pid_gains = {
        # The kp and kd gains of the PD control of the fingers.
        # Note: This depends on simulation step size and is set for a rate of 250 Hz.
        "P": [5.0, 5.0, 5.0] * _dims.NumFingers.value,
    ...
    def _pre_step(self):
        ...
        #print(self._dof_position.squeeze().detach())
    ...
    def __check_termination(self):
        ...
        self._step_info['env/current_position_dist/count'] = torch.mean(object_goal_position_dist)
        ...
        self._step_info['env/current_orientation_dist/count'] = torch.mean(object_goal_orientation_dist)
