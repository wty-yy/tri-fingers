...
class ObjectDistanceReward(RewardTerm):
    ...
    def compute(self, dt: float, curr_sched_step: float, object_state, goal_state) -> torch.Tensor:
        ...
        return self.weight * dt * sched_val * lgsk_kernel(dist)/100
...
class ObjectMoveReward(RewardTerm):
    ...
    def compute(self, object_state: torch.Tensor,
        ...
        return self.weight * (curr_norms - prev_norms)/100
...
class ObjectRotationReward(RewardTerm):
    ...
    def compute(self, dt: float, curr_sched_step: float, object_state: torch.Tensor,
        ...
        return self.weight * rot_rew/100
...
class ObjectRotationDeltaReward(RewardTerm):
    ...
    def compute(self, dt: float, curr_sched_step: float, object_state: torch.Tensor, last_object_state: torch.Tensor,
        ...
        return self.weight * rot_delta_rew/100
...
class FingerReachObjectRatePenalty(RewardTerm):
    ...
    def compute(self, curr_sched_step: float, fingertip_state: torch.Tensor, last_fingertip_state: torch.Tensor,
        ...
        ids=torch.nonzero((curr_norms-prev_norms)>0).squeeze().to(torch.int64).reshape(-1,2)  
        for i in range(ids.shape[0]):
            row_idx=ids[i,0]
            col_idx=ids[i,1]
            curr_norms[row_idx,col_idx]+=(curr_norms[row_idx,col_idx]-prev_norms[row_idx,col_idx])*1.4        

        return self.weight * sched_val * (curr_norms - prev_norms).sum(dim=-1)/100
...
class FingertipMovementPenalty(RewardTerm):
    ...
    def compute(self, dt: float, fingertip_state: torch.Tensor,
        ...
        return self.weight * fingertip_vel.pow(2).view(-1, 9).sum(dim=-1)/100
class FingerReachObjectReward(RewardTerm):
    """Reward encouraging movement of fingers towards the object."""

    def __init__(self, name: str = "finger_reach_object_dist", **kwargs):
        # norm type
        self._norm_p = kwargs.pop("norm_p", 2)
        # args
        activate = kwargs.pop("activate")
        ...
        # initialize base class
        super(FingerReachObjectReward, self).__init__(name, activate, weight)

    @torch.jit.export
    def compute(self,dt: float,curr_sched_step: float, fingertip_state: torch.Tensor, 
        ...
        if self.sched_enabled:
            ...
            sched_val = 1.0
        return self.weight * dt * sched_val * lgsk_kernel(curr_norms).sum(dim=-1)/100
...
REWARD_TERMS_MAPPING = {
    ...
    "finger_reach_object_dist":FingerReachObjectReward,
}